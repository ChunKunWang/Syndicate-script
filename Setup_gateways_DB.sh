...
docker run -d -t -i -p 31114:31114 --name rg-db syndicate-gateways
...
docker network connect Syndicate-Net rg-db
...
COMMAND="syndicate create_gateway email=$ADMIN_EMAIL volume=test-volume name=RG02 private_key=auto type=RG caps=ALL port=31114 host=rg-db"
docker exec -ti ms /bin/bash -c "$COMMAND"
...
COMMAND="yes y | sudo apt-get install vim"
docker exec -i ug-db /bin/bash -c "$COMMAND"
...
then
    cat cert.tar.gz | docker exec -i rg /bin/bash -c "$COPY_COMMAND"
    docker exec -i rg-db /bin/bash -c "$UNCOMPRESS_COMMAND"
fi
